...
def search(query):
    query = query.replace('+', ' ')
    songs = Song.objects.order_by('-listeners').filter(title__icontains=query)[:20] #fix this
    albums = Album.objects.order_by('-listeners').filter(title__icontains=query)[:5]
    artists = Artist.objects.order_by('-listeners').filter(name__icontains=query)[:5]
